...
|---|---|
| 1. Fire up the factory | `harness/factory.sh <run> "<task>"` — pins base SHA, records the task verbatim + digest, grounds (control 7), opens tmux session `<run>` with windows `ctl` (dispatcher) and `validator`. N factories = N runs; run state lives under `.harness/runs/<run>/`. |
| 2–5. Human ↔ Validator settle spec, architecture, test plan | The `validator` window runs `/validate` (Phase A0 research first). Artifacts are settled one behavior-ledger row at a time and land content-addressed in `.harness/runs/<run>/artifacts/` with `.digest` files. |
...
- `ground.sh` — control 7 + 9: resume from disk; reconcilers under `.harness/reconcile.d/`.
...
2. `.harness/schedule.registry` ships with a PROPOSED, UNRATIFIED workstation
