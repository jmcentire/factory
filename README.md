...
| `test_disposition.py` | The three-way disposition for a formerly passing test that now fails: an exact same-phase supersession plus an externally trusted affirmative human ruling over the run, current phase versions, exact old/new behavior, exact signed replacement statement, and frozen assertion/family permits a Tester-side update. Retained behavior requires fixing the implementation; silence/conflict routes to the human. The ruling acknowledges impact and cannot invent intent. | **Implemented** (pure classifier; signature verification and lane routing remain orchestration) |
...
|---|---|---|
| `state.py` | Hash-chained lifecycle from intake through promotion; `run.json` is only a checked projection. A `BUILDING` entry requires the complete frozen target/catalog/plan/input/readiness tuple and the next monotone attempt number. The ledger enforces the attempt ceiling; specification amendments clear all derived generation state. | The code has later states, but the shipped orchestrator intentionally stops at `preview`. Legacy v1 runs re-derive read-only and cannot advance under v2 controls. |
| `schema.py` + `schemas/` | Closed Draft 2020-12 contracts for authorized requests, genesis, receipts, the three invariant documents, pattern catalogs, build plans, human test-change rulings, and evidence bundles. | Semantic agreement remains human+Validator work. |
...
| `isolation.py` + `lanes.py` | On macOS, deny-default Seatbelt profiles give Coder and Tester disjoint read/write grants and no network. Both see the ratified build input; only Coder sees the recipe book/catalog; Validator sees both construction IR and frozen author outputs. Live forbidden read/write plus loopback bind/connect probes must pass. Validator alone runs the suite; automated repair receives only pass/fail. | This backend is macOS-only and fails closed elsewhere. Linux needs an independently qualified backend. Agent/model launch commands and narrow trusted-file grants are caller supplied. A tmux window or Kindex room name is not this control. |
| `evidence_plane.py` + `orchestrator.py` | Checklist items append when observed into a hash chain. Candidate and acceptance-test addresses are re-derived from retained review bytes, the validating transition binds both snapshot addresses, and every evidence bundle re-verifies the frozen generation tuple. The bundle records exact phases, test claims, attempt number/limit, generation addresses, review snapshots, lane/independence/model/directive facts, and monitors. A real Tessera test proves a failed first attempt, one clean retry, signed preview, and reproducibility after mutable source workspaces change. | Oracle-adequacy, determinism, independence, monitor, and correction records are Validator inputs at preview. Signed criticality-profile ingestion, target deployment/live probes, specialist review, human approval, CI handoff, RunStore promotion/rollback, correction baselines, monitor deployment/firing, and runtime tool/credential grants remain unwired. |
...
There is no chat-first collaborative phase-authoring loop, target deployment ladder, production
promotion, managed identity system, or live SSO/RBAC integration yet. The separate tmux harness
is a coordination/transition aid; its room names, Kindex channels, and prose fences are not a
substitute for the runtime's Seatbelt projection or signed ledger authority.
...
trusted human ruling names the exact assertion (or frozen family) and exact expected change.
...
5. After all three ratifications, a target-bound recipe book is verified and its exact generation
   inputs are retained. `FactoryOrchestrator` accepts caller-supplied agent launch commands and
   narrow trusted-file grants, runs Coder and Tester independently, freezes both outputs before
   review, lets only the Validator execute the acceptance suite, and writes the signed preview
   bundle. Attempts are bounded by both target ABI and plan. The executable
...
| `factory_core/test_disposition.py` | Pure authorization-based classification of formerly passing failures: update only on same-phase signed supersession plus an exact externally trusted human impact ruling; otherwise fix the code when authority is unchanged or route silence/conflict to the human. |
