...
| `tessera.py` + `authority.py` | Real Tessera Ed25519 envelope creation/validation; the exact linked envelope inode and containing directory are fsynced before `wrap_json` returns; externally pinned founder genesis; unique enrolled keys; exact run/repository/action/subject/signer/capability/expiry/nonce receipt checks. | Key rotation/revocation and managed HSM/KMS custody are not wired. Signet is not a prerequisite or authority here. |
